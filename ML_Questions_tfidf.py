import QuestionExtraction_tfidf
import re
import numpy as np
import pandas as pd
import time
...
df_new1 = QuestionExtraction_tfidf.ExtractData()

corpus = []
X =[]
y =[]
for each in df_new1:
    df_new = each[['File','ExtractedQuestions', 'QuestionCode']]


    eachText = ' '.join(np.asarray(df_new['ExtractedQuestions'].tolist()))
    corpus.append(str(eachText))

    X_old = np.asarray(df_new['ExtractedQuestions'].tolist())
    y1_old = np.asarray(df_new['QuestionCode'].tolist())
    y_old = np.asarray([1 if i=="C" else 0 for i in y1_old])
...
tfidf = TfidfVectorizer(ngram_range=(1,2),preprocessor=my_cool_preprocessor, max_features=10000)
...
for train_index, test_index in kf.split(X, y):
   ...
   score_precision = metrics.precision_score(y_test, y_pred, average=None)
   score_recall = metrics.recall_score(y_test, y_pred, average=None)
   score_f1 = metrics.f1_score(y_test, y_pred, average=None)
